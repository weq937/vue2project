<template>
  <div class="shop-header">
    <nav class="shop-nav">
      <a href="javascript:void(0)" class="back" @click="backs">
    ...
    <div class="shop-content">
      <img
        class="content-image"
        :src="
          `https://weq01.oss-cn-beijing.aliyuncs.com/` + shoplistone.logo
        "
      />
...
export default {
  ...
  async mounted() {
    // console.log('商家头部获取商家id',this.id)
    let id = this.id;
    //调用获取商家信息的接口
    let res = await shopone(id);
    // console.log(res)
    this.shoplistone = res;
  },
  methods:{
    backs(){
      this.$router.back();
